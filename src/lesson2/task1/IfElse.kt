...
fun segmentLength(a: Int, b: Int, c: Int, d: Int): Int = when {
    (a > d || b < c) -> -1                // пересечения отрезков нет
    (a <= d && d <= b && a <= c) -> d - c // пересечение отрезков есть
    (c <= b && b <= d && a < c) -> b - c  // пересечение отрезков есть
    (c <= a && b <= d) -> b - a           // пересечение отрезков есть
    else -> d - a                         // пересечение отрезков есть
}